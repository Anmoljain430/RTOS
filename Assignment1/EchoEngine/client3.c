...
{ 
	...
	while(1)
        {		
          printf("Enter a single character for client3 or ctrl+c to exit :\n");
	  message_send.msg_text = getchar(); 
          ...
          printf("Character sent from client3 is : %c \n", message_send.msg_text);
	  int temp; 
	  if (temp=msgrcv(msgid_rcv, &message_rcv, sizeof(message_rcv), 3, 0)==-1);
          {
	   getchar();	     
          }
