...
{ 
	...
	msgid_send = msgget(key[0], 0666 | IPC_CREAT);
       ...
       // User2 message_send
        message_send.msg_type = 5;
        // doubt of 2 times iterations
	while(1)
        {		
          printf("Enter a single character for client5 or ctrl+c to exit :\n");
	  message_send.msg_text = getchar(); 
          //gets(message_send.msg_text);
          // msgsnd to send message_send
          msgsnd(msgid_send, &message_send, sizeof(message_send), 5);

          // display the message_send
          printf("Character sent from client5  is : %c \n", message_send.msg_text);
	  int temp; 
	  if (temp=msgrcv(msgid_rcv, &message_rcv, sizeof(message_rcv), 5, 0)==-1);
          {
	   getchar();	     
          }
